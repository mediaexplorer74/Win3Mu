...
//            _moduleManager.LoadModule(new Shell());
        ...
        public int RunProgram(string programName, string commandL, int nCmdShow)
        {
            try
            {
                string[] commandArgs = {""};
                Utils.CommandLine cl = new Utils.CommandLine(commandArgs);

                // Store the program path
                ProgramHostPath = System.IO.Path.GetFullPath(programName);
                //ProgramHostPath = System.Environment.GetFolderPath(System.Environment.SpecialFolder.LocalApplicationData);

                // Work out base name for the program (we'll use this to merge program specific configuration settings)
                //string baseProgramName = ProgramHostPath + "\\" + programName.ToLowerInvariant();
                string baseProgramName =  System.IO.Path.GetFileNameWithoutExtension(programName).ToLowerInvariant();
                ...
                string programName16 = _pathMapper.MapHostToGuest(programName, false);
